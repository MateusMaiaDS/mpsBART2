...
new_sample <- no_interaction_friedman(n = 500,sd = sd_)
x_test <- new_sample$x
...
B_train_arr <- array(data = NA,
                     dim = c(nrow(x_train),
                             nrow(knots)+3, # +1 here because is a natural spline
...
if(dif_order!=0){
     ...
     P <- crossprod(D) + diag(1e-8,nrow = ncol(D))
     for(i in 1:length(continuous_vars)){
...
rmse(x = y_bart_hat,y = new_sample$y)

plot(x = y_test_hat,y = new_sample$y, main = "Test Spline")
plot(x = y_bart_hat,y = new_sample$y, main = "test BART")
